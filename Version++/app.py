...
def tasks_alpha():
    ...
    # Récupère les tâches de l'utilisateur triées par ordre alphabétique
    tasks = Task.query.filter_by(user_id=user.id).order_by(Task.title.asc()).all()
...
def tasks_page():
    ...
    # Récupère le numéro de page à afficher à partir de la requête
    page = request.args.get('page', 1, type=int)

    # Définit le nombre de tâches à afficher par page
    per_page = 5

    # Calcule l'offset en fonction du numéro de page
    offset = (page - 1) * per_page

    # Récupère les tâches de l'utilisateur avec la pagination
    tasks = Task.query.filter_by(user_id=user.id).order_by(Task.id.desc()).offset(offset).limit(per_page).all()
